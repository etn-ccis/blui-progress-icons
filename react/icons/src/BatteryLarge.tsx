...
export const BatteryLarge: React.FC<BatteryLargeProgressProps> = (props) => {
    ...
    const fillHeight = outlined ? (rangedPercent / 100) * 10 : (rangedPercent / 100) * 16;
    const fillY = outlined ? 8 + ((100 - rangedPercent) / 100) * 10 : 4 + ((100 - rangedPercent) / 100) * 16;
            ...
            <svg height={`${size}px`} width={`${size}px`} x="0px" y="0px" viewBox="0 0 24 24" {...svgProps}>
                ...
                {rangedPercent > 0 && (
                    <rect
                        ...
                        clipPath={`url(#${outlined ? outlinedClipId : clipId})`}
                    />
                )}
            </svg>
        </ProgressIcon>
